...
def _build_vocab(filename):
    ...
    # count_pairs are ("word", num_of_occurence) sorted from more occurent to less occurent
    count_pairs = sorted(counter.items(), key=lambda x: (-x[1], x[0]))

    #  words sorted from more occurent to less occurent (ties are broken alphabetically)
    words, _ = list(zip(*count_pairs))
    # dict associating a word and its id in the vocabulary
...
def _file_to_word_ids(filename, word_to_id):
    "reads a file and convert it to our vocabulary, if a word is not in the vocab then it is skipped"
    data = _read_words(filename)
...
def ptb_iterator(raw_data, batch_size, num_steps):
    ...
    # num_step is the length of the sequence on which we learn (number of time steps we use to compute the loss)
    # epoch_size is the number of times we compute the loss
...
def run_epoch(model, data, is_train=False, lr=1.0):
    ...
    """
    if is_train:
        # this is for dropout, batchNorm, etc. layers
        model.train()
    else:
        model.eval()
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        if args.model == 'TRANSFORMER':
            ...
            # actually transpose(0, 1) doesn't transpose the tensor. contiguous makes a copy of the tensor so that it is
            # contiguous in the memory (for the GPU I guess)
            inputs = torch.from_numpy(x.astype(np.int64)).transpose(0, 1).contiguous().to(device)#.cuda()
            model.zero_grad() # we set all the grads to zero
            hidden = repackage_hidden(hidden) # we detach the hidden variables from their history (we do not want to
                                              # backprop across batches)
            outputs, hidden = model(inputs, hidden)

        targets = torch.from_numpy(y.astype(np.int64)).transpose(0, 1).contiguous().to(device)#.cuda()
        tt = torch.squeeze(targets.view(-1, model.batch_size * model.seq_len)) # tt is now one long vector of index
        ...
        loss = loss_fn(outputs.contiguous().view(-1, model.vocab_size), tt) # for each word computes the cross-ent
                                                                            # the prediction (size of vocab) and the
                                                                            # correct index
        costs += loss.data.item() * model.seq_len # here we get sum of losses and not mean loss
        losses.append(costs)
        iters += model.seq_len
        if args.debug:
            print(step, loss)
            ...
            if args.optimizer == 'ADAM':
                ...
                for p in model.parameters():
                    if p.grad is not None:
                        p.data.add_(-lr, p.grad.data) # fancy in-place p = p - lr*p.grad
